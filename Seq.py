...
class Sequence:
    ...
    def __init__(self):
        ...
        self.alph2= ["A","C", "D", "E","F", "G", "H", "I", "K", "L", "M", "N", "P","Q","R", "S", "T", "V","W", "Y", "-"] #Using this when sequences are not the same length in the data ( in our case affibody sequences are equal length and NESP are different)
    ...
    def encode_one_hot_padded(self, max_length): #Onehot encoding the sequences, spaerse representations
         
        one_hot_length = len(self.alph2)
        one_hots=[]
        m = self.df['Sequence'].str.len().max()
        self.df['Sequence']=self.df['Sequence'].str.ljust(m, '-')
        for i in range(one_hot_length):
            a = np.zeros(one_hot_length)
            a[i] = 1
            one_hots.append(a)
    ...
    def score_concat_reps_over_sampling_optuna(self,trial, lr_list, decay_list, batch_size_list, epochs_list,fold_n,random_state,embedding_concat_list,sampling_method):  #if include onehot, bring at the end of the concat list
    ...
    def LR_NESP(self, embedding_type, protein_length, sample_size,test_ratio, random_state):
        
        self.df= self.df[self.df['length']<=protein_length] 
        m = self.df['Sequence'].str.len().max()
        
        self.df['Sequence']=self.df['Sequence'].str.ljust(m, '-')
        self.encode_one_hot_padded(protein_length,m)
        df_unstable=self.df[self.df['label']==0].sample(sample_size,random_state=random_state)
        df_stable=self.df[self.df['label']==1].sample(sample_size,random_state=random_state)
        self.df_stable= df_stable
        dff= pd.concat([df_stable,df_unstable]).sample(frac=1, random_state= random_state)
        list_rep=[]
        
        for i in dff[embedding_type]:
            if embedding_type=='onehot' or embedding_type=='physical' :
               list_rep.append(i)
            else:
               list_rep.append(np.fromstring(i[1:-1], dtype=float, sep=','))
        self.list_rep=list_rep
        X_train, X_test, Y_train, Y_test = train_test_split(list_rep,dff['label'], test_size=test_ratio,random_state= random_state)
        logistic_model = LogisticRegression(penalty='l2',max_iter=2000)
        logistic_model.fit(X_train, Y_train)
        Y_pred = logistic_model.predict(X_test)
        Y_pred_prob=logistic_model.predict_proba(X_test)
        f1_score_= f1_score(Y_test,Y_pred)*100
    ...
    def Concat_LR_NESP(self,embedding_concat_list, protein_length, sample_size,test_ratio, random_state):
        self.df= self.df[self.df['length']<=protein_length] 
        
        self.encode_one_hot_padded(protein_length, protein_length)
        df_unstable=self.df[self.df['label']==0].sample(sample_size,random_state=random_state)
        df_stable=self.df[self.df['label']==1].sample(sample_size,random_state=random_state)
        self.df_stable= df_stable
        dff= pd.concat([df_stable,df_unstable]).sample(frac=1, random_state= random_state)
        ...
        if len(embedding_concat_list)<3:
             # The embeddings were saved as strings and I am changing them back to numeric here
            for (i, j) in zip(dff[embedding_concat_list[0]],dff[embedding_concat_list[1]]):
                if embedding_concat_list[-1] !='onehot':
                    ll=np.concatenate([np.fromstring(i[1:-1], dtype=float, sep=','),np.fromstring(j[1:-1], dtype=float, sep=',')],axis=0).reshape(1,len(np.fromstring(i[1:-1], dtype=float, sep=','))+len(np.fromstring(j[1:-1], dtype=float, sep=',')))             
                    list_rep.append(ll)
                else:
                    list_rep.append(np.concatenate([np.fromstring(i[1:-1], dtype=float, sep=','),j], axis=0).reshape(1,len(np.fromstring(i[1:-1], dtype=float, sep=','))+len(j)))
            X= np.array(list_rep) 
            X= X.reshape(X.shape[0], X.shape[-1])
                  
        else: 
            for (i, j, k) in zip(dff[embedding_concat_list[0]],dff[embedding_concat_list[1]], dff[embedding_concat_list[2]]):
          
                list_rep.append(np.concatenate([np.fromstring(i[1:-1], dtype=float, sep=','),np.fromstring(j[1:-1], dtype=float, sep=','),k], axis=0).reshape(1,len(np.fromstring(i[1:-1], dtype=float, sep=','))+len(np.fromstring(j[1:-1], dtype=float, sep=','))+len(k)))
            X= np.array(list_rep)
            X= X.reshape(X.shape[0], X.shape[-1])
            
        X_train, X_test, Y_train, Y_test = train_test_split(X,dff['label'], test_size=test_ratio,random_state= random_state)
        logistic_model = LogisticRegression(penalty='l2',max_iter=2000)
        logistic_model.fit(X_train, Y_train)
        Y_pred = logistic_model.predict(X_test)
        f1_score_= f1_score(Y_test,Y_pred)*100
        confusion_mat = confusion_matrix(Y_test,Y_pred)
        return f1_score_, confusion_mat, Y_pred, Y_test
                  
    def score_model_RF_Reg_NESP(self, trial, embed_type, protein_length, fold_n,max_depth_list, n_estimators_list, random_state):
  # function for random forest regression on NESP, optimized by Optuna ( minimizing MSE in validation set) 
  
        r2=[]
        MSE=[]
        r2_test=[]
        MSE_test=[]
        X= self.df[embed_type]
        X=pd.DataFrame(X)
        y= self.df['tm']
        cv= KFold(n_splits=fold_n)
        embed_holder=[]
        for i in list(X[embed_type]):
            if embed_type =='onehot':
                  embed_holder.append(i)
            else:
                  embed_holder.append(np.fromstring(i[1:-1], dtype=float, sep=','))
           
        X_,X_test, y_, y_test = train_test_split(pd.DataFrame(embed_holder).reset_index(drop=True),y.reset_index(drop=True), test_size = 0.2, random_state=random_state)    
        print(X_.sort_index() ,y_.sort_index())
        for train_index, val_index in cv.split(pd.DataFrame(X_)):
            X_train, X_val, y_train, y_val = X_.iloc[train_index], X_.iloc[val_index], y_.iloc[train_index], y_.iloc[val_index]
...
# Splitting the data into training and testing 
            ...
            if trial==None:
       	        regr=sklearn.ensemble.RandomForestRegressor(n_estimators=n_estimators_list, max_depth=max_depth_list)
                regr.fit(X_train,y_train)

            else:   

 # Defining the parameters
                rf_n_estimators = trial.suggest_int("rf_n_estimators", n_estimators_list[0],n_estimators_list[1] )  
                rf_max_depth = trial.suggest_int("rf_max_depth", max_depth_list[0],max_depth_list[1] , log=True)
                regr=sklearn.ensemble.RandomForestRegressor(n_estimators= rf_n_estimators,max_depth=rf_max_depth)
                regr.fit(X_train,y_train)
        ...
        if trial==None:
            return list( y_test), list(y_pred), MSE, r2, MSE_test, r2_test
